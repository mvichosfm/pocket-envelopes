...
4. **Fund the month** on the Envelopes tab, once income has landed. It assigns each envelope its budgeted amount and shows, live, what that does to the forecast's lowest point. Mid-month, each envelope's **Fund** button sets money aside in that one envelope — the reserve included — and **Use projected low** (or **Fund reserve** on the dashboard) parks the most you can before the forecast's low reaches zero.
